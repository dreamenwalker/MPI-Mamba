

# 仿真数据下载及说明

##汉字仿真数据
该数据位网络下载的数据
data-hanzi.rar数据链接：
说明：该数据集是用于生成MPI数据的原始数据。数据下载链接如下：
 https://pan.baidu.com/s/19SRcUta3cTlw9sSXxJTKzw?pwd=fufi 提取码: fufi
 https://drive.google.com/file/d/1m1oPl0DOz7nFdGF9nyFvMGkSiLxkr6Px/view?usp=drive_link
MPI仿真数据链接如下：
文件说明：data-hanzi_result5.rar，为基于MPI仿真程序获得的MPI数据：
链接: https://pan.baidu.com/s/1mi60ji5tyyPcyWku1veD-A?pwd=pkuu 提取码: pkuu

## 多种形状的仿真数据，包含标签和原图：
链接: https://pan.baidu.com/s/109aD9JJgg3tqPMfVl8PMdg?pwd=ef5e 提取码: ef5e

## 在各向异性设置参数后的仿真数据
说明：该数据已经进行好划分，label为对应的标签
通过网盘分享的文件：data_split.zip
链接: https://pan.baidu.com/s/1_NPxCe5Jvj_L05brTKQeWg?pwd=pp8x 提取码: pp8x

## 在 6T下的仿真数据

链接: https://pan.baidu.com/s/1QNecO3ODbzkneiwXp-AN8Q?pwd=kyas 提取码: kyas
